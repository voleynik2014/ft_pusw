...
/*   Updated: 2017/03/21 15:43:38 by voliynik         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ps->a_len >= 2 && (*tab_a)[0] > ps->med &&
			(*tab_a)[1] <= ps->med && !ft_check_next(tab_a, ps->med, size, 2))
	{
		...
		if (!((*tab_a)[0] >= (*tab_b)[0] && (*tab_a)[0] >= (*tab_b)[1] && (size_t)size == ps->a_len))
		{
...
int		ft_sort_less_6(int **tab_a, int **tab_b, size_t size, t_st *ps)
{
	...
	while (1)
	{
		...
		else if ((i = ft_sort_last_param(tab_a, tab_b, size, ps)))// && ++res))
		{
		i == 1 ? ++res : 0;
			break ;
		}
...
int		ft_check_next_b(int **tab_a, int med, int size, int fl)
{
	int i;
...
int		ft_sort_last_param_b(int **tab_a, int **tab_b, int size, t_st *ps)
{
	if ((*tab_a)[0] <= ps->med && (*tab_a)[1] > ps->med
			&& !ft_check_next_b(tab_a, ps->med, size, 2))
	{
		if ((*tab_b)[0] > (*tab_b)[1] && (*tab_a)[0] < (*tab_a)[1])
			ft_swap_both(tab_a, tab_b, ps);
		else
			ft_swap_first(tab_a, ps, 2);
...
int		ft_sort_less_6_b(int **tab_a, int **tab_b, int size, t_st *ps)
{
	...
	while (++i < size)
	{
		if (!ft_check_next_b(tab_a, ps->med, size, 0))
			break ;
		if (res >= 2 && (*tab_b)[0] > (*tab_b)[1] && (*tab_a)[0] < (*tab_a)[1])
			ft_swap_both(tab_a, tab_b, ps);
		else if (res >= 2 && (*tab_b)[0] > (*tab_b)[1])
			ft_swap_first(tab_b, ps, 1);
		else if ((*tab_a)[0] > ps->med && ++res)
			ft_put_dig_in_a(tab_a, tab_b, ps);
		else if (ft_sort_last_param_b(tab_a, tab_b, size, ps) && ++res)
			break ;
		else if (++ps->rot)
			ft_rotate(tab_a, ps->b_len, ps, 2);
	}
	if (tmp == size)
		ps->rot = 0;
	return (res);
